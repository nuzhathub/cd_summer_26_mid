    ...
    for(int i=0;i<inputLine.length();i++){
        ...
        if(inputLine[i]=='"'){
            ...
            while(true){
                ...
                currentToken+=inputLine[n];
                n++;
            }

            tokenList[index]=currentToken+" -> Valid String Constant";
            index++;
            currentToken="";
            i=n;
            continue;
        ...
        if(inputLine[i]==' ' || i==(inputLine.length()-1)){
            ...
            for(int j=0;j<3;j++){
                ...
                if(currentToken==reservedWords[j]){
                    ...
                    tokenList[index]=currentToken+" -> Valid Keyword";
                    index++;
                    currentToken="";
                }
                else if(currentToken==operators[j]){

                    tokenList[index]=currentToken+" -> Valid Operator";
                    index++;
                    currentToken="";
                }
                else if(currentToken==punctuators[j]){

                    tokenList[index]=currentToken+" -> Valid Punctuator";
                    index++;
                    currentToken="";
                }
            }
...
int main(){
    ...
    string code = R"(cout << 60 + 20.5 << "hello world" << endl ;)";
